...
Yes. To use profile script, wrap the name in `\${...}`. For example, if your profile script is named `user.basket`, in design, you can refer it as `\${user.basket}`. Note, the backslash implies that the profile script is not rendered by Velocity. Therefore, you cannot perform any operations on it in Velocity template. The value will be directly printed on the page.
